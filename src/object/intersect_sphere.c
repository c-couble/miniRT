...
/*   Updated: 2024/08/28 04:34:04 by ccouble          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	fill_uv(t_object *obj, t_ray *ray)
{
	ray->data.u = 0.5 + (atan2(ray->data.normal.y, ray->data.normal.x) / (M_PI * 2));
	ray->data.v = 0.5 - (asin(ray->data.normal.z / obj->data.sphere.radius) / M_PI);
}
