...
/*   Updated: 2026/02/05 01:02:22 by lespenel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_colorf		scene_light = color_normalize(in->color);

	scene_light = color_scale(scene_light, in->ratio);
	
	difuse_reflect(&scene_light, &l_ray->ray, c_ray);
	*out = color_add_scale(*out, scene_light, c_ray->data.materials->diffuse_ratio);
	specular_reflect(&scene_light, l_ray, c_ray);
	*out = color_add_scale(*out, scene_light, c_ray->data.materials->specular_ratio);
}
